package com.genersoft.iot.wvmp.service.redisMsg;
...
import com.genersoft.iot.wvmp.service.bean.*;
...
public class RedisGbPlayMsgListener implements MessageListener {
    ...
    public interface PlayMsgErrorCallback{
        void handler(com.genersoft.iot.wvmp.vmanager.bean.WvmpResult wvmpResult);
    }
        ...
        if (!taskQueueHandlerRun) {
            ...
            taskExecutor.execute(() -> {
                while (!taskQueue.isEmpty()) {
                    ...
                    WvmpRedisMsg wvmpRedisMsg = JSON.toJavaObject(msgJSON, WvmpRedisMsg.class);
                    if (!userSetting.getServerId().equals(wvmpRedisMsg.getToId())) {
                        continue;
                    }
                    if (WvmpRedisMsg.isRequest(wvmpRedisMsg)) {
                        logger.info("[收到REDIS通知] 请求： {}", new String(msg.getBody()));

                        switch (wvmpRedisMsg.getCmd()){
                            case WvmpRedisMsgCmd.GET_SEND_ITEM:
                                RequestSendItemMsg content = JSON.toJavaObject((JSONObject) wvmpRedisMsg.getContent(), RequestSendItemMsg.class);
                                requestSendItemMsgHand(content, wvmpRedisMsg.getFromId(), wvmpRedisMsg.getSerial());
                                break;
                            case WvmpRedisMsgCmd.REQUEST_PUSH_STREAM:
                                RequestPushStreamMsg param = JSON.toJavaObject((JSONObject) wvmpRedisMsg.getContent(), RequestPushStreamMsg.class);;
                                requestPushStreamMsgHand(param, wvmpRedisMsg.getFromId(), wvmpRedisMsg.getSerial());
                                break;
                            default:
                                break;
                        }
                        ...
                        switch (wvmpRedisMsg.getCmd()){
                            case WvmpRedisMsgCmd.GET_SEND_ITEM:
                                ...
                                com.genersoft.iot.wvmp.vmanager.bean.WvmpResult content  = JSON.toJavaObject((JSONObject) wvmpRedisMsg.getContent(), com.genersoft.iot.wvmp.vmanager.bean.WvmpResult.class);

                                String key = wvmpRedisMsg.getSerial();
                                ...
                                break;
                            case WvmpRedisMsgCmd.REQUEST_PUSH_STREAM:
                                com.genersoft.iot.wvmp.vmanager.bean.WvmpResult wvmpResult = JSON.toJavaObject((JSONObject) wvmpRedisMsg.getContent(), com.genersoft.iot.wvmp.vmanager.bean.WvmpResult.class);
                                String serial = wvmpRedisMsg.getSerial();
                                switch (wvmpResult.getCode()) {
                                    case 0:
                                        JSONObject jsonObject = (JSONObject) wvmpResult.getData();
                                    ...
                                    case ERROR_CODE_TIMEOUT:
                                        ...
                                        if (errorCallback != null) {
                                            ...
                                            errorCallback.handler(wvmpResult);
                                        }
                                        break;
                                    default:
                                        break;
    ...
    private void responsePushStream(JSONObject content, String toId, String serial) {
        ...
        com.genersoft.iot.wvmp.vmanager.bean.WvmpResult<JSONObject> result = new com.genersoft.iot.wvmp.vmanager.bean.WvmpResult<>();
        result.setCode(0);
        result.setData(content);

        WvmpRedisMsg response = WvmpRedisMsg.getResponseInstance(userSetting.getServerId(), toId,
                WvmpRedisMsgCmd.REQUEST_PUSH_STREAM, serial, result);
    ...
    private void requestSendItemMsgHand(RequestSendItemMsg content, String toId, String serial) {
        ...
        if (mediaServerItem == null) {
            ...
            com.genersoft.iot.wvmp.vmanager.bean.WvmpResult<SendRtpItem> result = new com.genersoft.iot.wvmp.vmanager.bean.WvmpResult<>();
            result.setCode(ERROR_CODE_MEDIA_SERVER_NOT_FOUND);
            result.setMsg("流媒体不存在");

            WvmpRedisMsg response = WvmpRedisMsg.getResponseInstance(userSetting.getServerId(), toId,
                    WvmpRedisMsgCmd.GET_SEND_ITEM, serial, result);
        ...
        if (streamReady) {
            ...
            dynamicTask.startDelay(taskKey, ()->{
                ...
                WvmpRedisMsg response = WvmpRedisMsg.getResponseInstance(
                        userSetting.getServerId(), toId, WvmpRedisMsgCmd.GET_SEND_ITEM, serial, result
                );
                JSONObject jsonObject = (JSONObject)JSON.toJSON(response);
                RedisUtil.convertAndSend(WVP_PUSH_STREAM_KEY, jsonObject);
    ...
    private void responseSendItem(MediaServerItem mediaServerItem, RequestSendItemMsg content, String toId, String serial) {
        ...
        com.genersoft.iot.wvmp.vmanager.bean.WvmpResult<ResponseSendItemMsg> result = new com.genersoft.iot.wvmp.vmanager.bean.WvmpResult<>();
        result.setCode(0);
    ...
    public void sendMsg(String serverId, String mediaServerId, String app, String stream, String ip, int port, String ssrc,
        ...
        WvmpRedisMsg redisMsg = WvmpRedisMsg.getRequestInstance(userSetting.getServerId(), serverId, WvmpRedisMsgCmd.GET_SEND_ITEM,
        ...
        dynamicTask.startDelay(key, ()->{
            callbacks.remove(key);
            callbacksForError.remove(key);
            com.genersoft.iot.wvmp.vmanager.bean.WvmpResult<Object> wvmpResult = new com.genersoft.iot.wvmp.vmanager.bean.WvmpResult<>();
            wvmpResult.setCode(ERROR_CODE_TIMEOUT);
            wvmpResult.setMsg("timeout");
            errorCallback.handler(wvmpResult);
        }, userSetting.getPlatformPlayTimeout());
        RedisUtil.convertAndSend(WVP_PUSH_STREAM_KEY, jsonObject);
    }
    ...
    public void sendMsgForStartSendRtpStream(String serverId, RequestPushStreamMsg param, PlayMsgCallbackForStartSendRtpStream callback) {
        String key = UUID.randomUUID().toString();
        WvmpRedisMsg redisMsg = WvmpRedisMsg.getRequestInstance(userSetting.getServerId(), serverId,
                WvmpRedisMsgCmd.REQUEST_PUSH_STREAM, key, param);

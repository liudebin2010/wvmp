package com.genersoft.iot.wvmp.gb28181.protocol.device;
...
import com.genersoft.iot.wvmp.gb28181.bean.SubscribeHolder;
import com.genersoft.iot.wvmp.gb28181.bean.SyncStatus;
import com.genersoft.iot.wvmp.gb28181.task.ISubscribeTask;
import com.genersoft.iot.wvmp.gb28181.task.impl.CatalogSubscribeTask;
import com.genersoft.iot.wvmp.gb28181.task.impl.MobilePositionSubscribeTask;
...
import com.genersoft.iot.wvmp.vmanager.bean.BaseTree;
...
public class DeviceQuery {
	...
	public WvmpResult<SyncStatus> devicesSync(@PathVariable String deviceId){
		...
		if (status) {
			...
			return WvmpResult.success(channelSyncStatus);
		}
		deviceService.sync(device);

		WvmpResult<SyncStatus> wvpResult = new WvmpResult<>();
		wvpResult.setCode(0);
	...
	public ResponseEntity<WvmpResult<String>> updateDevice(Device device){

		if (device != null && device.getDeviceId() != null) {
			deviceService.updateDevice(device);
		}
		WvmpResult<String> result = new WvmpResult<>();
		result.setCode(0);
	...
	@Parameter(name = "deviceId", description = "设备国标编号", required = true)
	public WvmpResult<SyncStatus> getSyncStatus(@PathVariable String deviceId) {
		SyncStatus channelSyncStatus = deviceService.getChannelSyncStatus(deviceId);
		WvmpResult<SyncStatus> wvpResult = new WvmpResult<>();
	...
	@Parameter(name = "deviceId", description = "设备国标编号", required = true)
	public WvmpResult<Map<String, Integer>> getSubscribeInfo(@PathVariable String deviceId) {
		Set<String> allKeys = dynamicTask.getAllKeys();
		...
		WvmpResult<Map<String, Integer>> wvpResult = new WvmpResult<>();
		wvpResult.setCode(0);

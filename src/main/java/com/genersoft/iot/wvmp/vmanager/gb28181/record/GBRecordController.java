package com.genersoft.iot.wvmp.vmanager.gb28181.record;
...
import com.genersoft.iot.wvmp.gb28181.bean.RecordInfo;
...
public class GBRecordController {
	...
	public DeferredResult<WvmpResult<RecordInfo>> recordinfo(@PathVariable String deviceId, @PathVariable String channelId, String startTime, String endTime){

		if (logger.isDebugEnabled()) {
			logger.debug(String.format("录像信息查询 API调用，deviceId：%s ，startTime：%s， endTime：%s",deviceId, startTime, endTime));
		}
		DeferredResult<WvmpResult<RecordInfo>> result = new DeferredResult<>();
		...
		try {
			cmder.recordInfoQuery(device, channelId, startTime, endTime, sn, null, null, null, (eventResult -> {
				WvmpResult<RecordInfo> wvpResult = new WvmpResult<>();
	...
	public DeferredResult<WvmpResult<StreamInfo>> download(@PathVariable String deviceId, @PathVariable String channelId,
		...
		DeferredResult<WvmpResult<StreamInfo>> result = playService.download(deviceId, channelId, startTime, endTime, Integer.parseInt(downloadSpeed), null, hookCallBack->{
